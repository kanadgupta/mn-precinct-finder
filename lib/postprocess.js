...
precincts
  .forEach((precinct) => {
    ...
    if (fipsCode.length !== 3)
      console.error("Invalid length for county code:", precinct);

    const precinctCode = precinct["Precinct Code"];

    if (precinctCode.length !== 4)
      console.error("Invalid length for precinct code:", precinct);

    const fullPrecinctCode = `${stateCode}${fipsCode}${precinctCode}`;

    if (fullPrecinctCode.length !== 9)
      console.error("Invalid length for full precinct code:", precinct);
